...
class TransformerAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent_args.add_argument(
            "--max_history", type=int, default=2,
        ...
        agent_args.add_argument(
            "--eval_type", type=str, default="hits@1",
            help="hits@1, ppl or f1"
        )
    ...
    def __init__(self, opt, shared=None):
        ...
        if shared is None:
            if args.model_checkpoint == "":
                args.model_checkpoint = bot_utils.download_pretrained_model()
    ...
    def observe(self, observation):
        ...
        if "labels" in observation or "eval_labels" in observation:
            text = observation.get(
                "labels", observation.get("eval_labels", [[]]))[0]
        ...
        if "label_candidates" in observation:
            for candidate in observation["label_candidates"]:
                candidates.append(
                    (self.tokenizer.encode(candidate), candidate))
    ...
    def act(self):
        ...
        if self.args.eval_type == "hits@1" and len(self.candidates) > 0:
            ...
            inputs = pad_dataset(
                instances, padding=self.special_tokens_ids[-1])

            tensor_inputs = {}
            for input_name in ["input_ids", "mc_token_ids", "token_type_ids"]:
                tensor = torch.tensor(
                    inputs[input_name], device=self.args.device)
                tensor = tensor.view(
                    (-1, len(self.candidates)) + tensor.shape[1:])
    ...
    def next_word_probability(self, partial_out):
        ...
        token_type_ids = torch.tensor(
            instance["token_type_ids"], device=self.args.device).unsqueeze(0)

        with torch.no_grad():
            logits = self.model_checkpoint(
